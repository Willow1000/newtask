...
def get_font(family="Trebuchet MS", size=14, weight="normal", slant="roman", underline=False):
    ...
    """
    return customtkinter.CTkFont(family=family, size=size, weight=weight, slant=slant, underline=underline)
...
class SpriteScraperView(customtkinter.CTkFrame):
    def __init__(self, action_id, data_path, parent):
    ...
    def on_submit(self):
        ...
        thread = threading.Thread(
            ...
            kwargs={"search_string": search_string, "image_type": self.radio_var.get(), "notify_callback": self.update_log, 'action_id': self.action_id, 'data_path': self.data_path},
            daemon=True,
        )
