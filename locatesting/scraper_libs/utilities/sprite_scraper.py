...
from . import imagesearch as imsearch
...
class SpriteScraper:
    ...
    def search_and_download(self, search_string: str, **kwargs) -> Path:
        ...
        for img_name in img_names:
            ...
            if not did_succeed:
                ...
                print("success downloading and saving image")
                notify_callback("success downloading and saving image")
    ...
    def __extract_kwargs(self, kwargs):
        ...
        destination = f'{ data_path }/all_action_files/screenshots/action/'
    ...
    def __get_item_infobox_data(self, item: str) -> Optional[str]:
        ...
        try:
            response = requests.get(url=f"{self.BASE_URL}/api.php", params=params, headers=headers)
            print(response.url)
            data = response.json()
    ...
    def __sprite_url(self, item: str) -> str:
        ...
        if match := re.search(pattern, info_box):
            ...
            print(f"DEBUGPRINT[4]: sprite_scraper.py:198: image_url={image_url}")
            return image_url
        else:
    ...
    def __save_image(self, img_name: str, downloaded_img: np.ndarray, image_type: ImageType, destination: str, notify_callback) -> bool:
        ...
        try:
            if image_type in {ImageType.NORMAL, ImageType.ALL}:
                print(f"file path I am saving image to={filepath}.png")
            ...
            if image_type in {ImageType.BANK, ImageType.ALL}:
                print(f"file path I am saving image to={filepath}_bank.png")
