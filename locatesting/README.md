...
docker 
discord
XLaunch (https://sourceforge.net/projects/vcxsrv/) or xorg if you are on linux
on Linux: 
python3: 
 (python version 3.10 I believe)
 sudo apt install -y python3 python3-pip
xorg and xephyr: (instead of XLaunch)
 sudo apt-get install xorg -y
 sudo apt-get install xserver-xephyr -y
some other dependencies on linux:
 ...
 *"your_first_name" is the name of the account you will be provided, which will probably be your actual first name
 *"your_keyname" is just {your_first_name}_key
 4.1 to sync files:
  in the line below you must replace your_first_name with your username on the system, which should be your first name, sync_pairs_dir, your_dev_key_path
   sudo rsync -avz --progress --delete-after -e "ssh -i "key_path" -o ServerAliveCountMax=3 -o ConnectTimeout=10 -o TCPKeepAlive=yes" "actions_folder"/ "your_username"@116.202.29.166:dev_code; 
  afterwards you need to send the update file, with the command:
   touch /tmp/update; sudo rsync -avz --progress --delete-after -e "ssh -i "key_path" -o ServerAliveCountMax=3 -o ConnectTimeout=10 -o TCPKeepAlive=yes" /tmp/update "your_username"@116.202.29.166:update; rm /tmp/update 
  you probably just want to use this commands together:
   sudo rsync -avz --progress --delete-after -e "ssh -i "key_path" -o ServerAliveCountMax=3 -o ConnectTimeout=10 -o TCPKeepAlive=yes" "actions_folder"/ "your_username"@116.202.29.166:dev_code;touch /tmp/update; sudo rsync -avz --progress --delete-after -e "ssh -i "key_path" -o ServerAliveCountMax=3 -o ConnectTimeout=10 -o TCPKeepAlive=yes" /tmp/update "your_username"@116.202.29.166:update; rm /tmp/update  
  *you need to send the update file for the server to read your updated files, so always make sure to send it, whenever you  
 ...
 4.2 synchronize your scheduler_updates 
...
# recommended is: 
   scheduler_updates_folder=LOCAL_DIRECTORY/scheduler_updates_dir (the path to the directory containing the scheduler update), and keep everything in one directory
   actions_folder=LOCAL_DIRECTORY/runescape_actions
   
4.3 How do you send your scheduler updates to the server:
...
and then you run the command at 4.2 or just chain them together
