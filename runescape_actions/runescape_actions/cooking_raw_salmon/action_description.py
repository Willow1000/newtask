...
current_action_id = "cooking_raw_salmon"
app_config_id = "full_rs"  # each action may require a different set of configs from the app itself
context = "rs_ps"  # context to know what profile to use, what is this session related to, etc.

# Step 1: Cook (raw salmon)

updates = [
    {
        "id": "click_fish",
        "check": get_action_picture_by_name("raw_salmon"),
        "verify": get_action_picture_by_name("raw_salmon_selected"),
        "test": [
            {
            "mock_image": get_test_picture_by_name("test_raw_salmon"),  
            },
        ],
        "extra_test_info": {
        "end_mock_image_list": [
            get_test_picture_by_name("test_raw_salmon")
            ],
        },
    },
    {
        "id": "click_fish_icon",
        "check": get_action_picture_by_name("raw_salmon_icon"), 
        "test": [
            {
...
cook_raw_salmon = get_cook_fish(updates)
action_ordered_steps = cook_raw_salmon

# Step 8: Deposit cooked salmon (all)

deposit_cooked_salmon = get_deposit_all("salmon", "test_salmon")
action_ordered_steps += copy.deepcopy( deposit_cooked_salmon )
