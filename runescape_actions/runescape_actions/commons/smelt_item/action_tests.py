...
class SmeltItemTest(Test):
    ...
    def hooks_setup(self):
        hooks = {
            'click_bar': { 
                "check": self.hook_click_bar,
            },
            'click_furnace': { 
                "check": self.hook_shift_cick,
            },
            
        }
        self.set_hooks(hooks)

    def hook_click_bar(self, function_received, args_to_func_received, test_element):
        '''
        ...
        '''
        print('hooking click_bar id step') 
        print(args_to_func_received)
        #self.basic_mock_test(function_received, args_to_func_received, test_element)
    
    def hook_shift_cick(self, function_received, args_to_func_received, test_element):
        print('hooking click_furnace id step') 
        print(args_to_func_received)
        #self.basic_mock_test(function_received, args_to_func_received, test_element)

    def run(self):
        print('testing smelt item')
        # import everything you need from action_description here
        from smelt_item.action_description import time_limit, current_action_id, \
            app_config_id, context 
        # from action_description import action_ordered_steps
        from smelt_item.action_description import smelt_item_test
        action_ordered_steps = smelt_item_test()
